...
const LINE_TYPES: InvoiceLineType[] = [
  "TRANSPORT", "STORAGE", "INSURANCE", "DELIVERY", "HANDLING", "OTHER"
];

const lineItemSchema = z.object({
  type: z.enum(["TRANSPORT", "STORAGE", "INSURANCE", "DELIVERY", "HANDLING", "OTHER"]),
...
const STATUS_TONE: Record<InvoiceRecord["status"], "neutral" | "success" | "warning" | "danger"> = {
  DRAFT: "neutral",
  ISSUED: "neutral",
...
export default function InvoicesPage() {
  ...
  const { data, error, isLoading, mutate: refetch } = useSWR(
    swrKey,
    () => backofficeApi.listInvoices({ page, pageSize: 10 })
  );
  ...
  const {
    ...
    defaultValues: {
      currencyCode: "DOP",
      lines: [{ type: "TRANSPORT" as const, description: "", quantity: "1", unitPriceMinor: "0" }],
    },
  });
  ...
  async function onSubmit(values: CreateInvoiceForm) {
    try {
      ...
      pushToast(err instanceof ApiError ? err.message : "No fue posible crear la factura.");
    }
  }
    ...
    <div className="page-stack">
      ...
      <Card>
        {isLoading ? (
          ...
          <ErrorState title="Error al cargar facturas" description={error.message} onRetry={() => void refetch()} />
        ) : invoices.length === 0 ? (
          <EmptyState title="No hay facturas" description="Crea la primera factura usando el botón 'Nueva factura'." />
        ) : (
          <>
            <Table
              columns={["Nº Factura", "Estado", "Total", "Balance Pendiente", "Fecha"]}
              rows={invoices.map((inv) => [
                <span key={`num-${inv.id}`} className="inline-code">{inv.invoiceNumber}</span>,
                <Badge key={`st-${inv.id}`} tone={STATUS_TONE[inv.status]}>{STATUS_LABEL[inv.status]}</Badge>,
      ...
      >
        <form id="create-invoice-form" className="form-grid" onSubmit={handleSubmit(onSubmit)}>
          ...
          <div>
            ...
            {errors.lines && typeof errors.lines === "object" && !Array.isArray(errors.lines) && (
              <span className="ui-field__error">{(errors.lines as { message?: string }).message}</span>
            )}
          </div>

          {fields.map((field, idx) => (
            <div key={field.id} className="form-grid" style={{ border: "1px solid #e0e8ee", borderRadius: 8, padding: 12 }}>
              <FormField label="Tipo" error={errors.lines?.[idx]?.type?.message}>
                <Select {...register(`lines.${idx}.type`)}>
                  {LINE_TYPES.map((t) => <option key={t} value={t}>{t}</option>)}
                </Select>
              </FormField>
              <FormField label="Descripción" error={errors.lines?.[idx]?.description?.message}>
                <Input {...register(`lines.${idx}.description`)} />
              </FormField>
              <FormField label="Cantidad" error={errors.lines?.[idx]?.quantity?.message}>
                <Input type="number" min={1} {...register(`lines.${idx}.quantity`)} />
              </FormField>
              <FormField label="Precio unitario" error={errors.lines?.[idx]?.unitPriceMinor?.message}>
                <Input type="number" step="0.01" min={0} {...register(`lines.${idx}.unitPriceMinor`)} />
              </FormField>
              {fields.length > 1 && (
          ...
          <Button
            variant="secondary"
            type="button"
            onClick={() => append({ type: "TRANSPORT", description: "", quantity: "1", unitPriceMinor: "0" })}
          >
            + Agregar línea
          </Button>
        </form>
