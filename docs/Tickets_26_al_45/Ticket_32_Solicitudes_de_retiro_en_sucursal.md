# Ticket 32 — Solicitudes de retiro en sucursal

## Rama y commit

```powershell
git switch main
git switch -c feat/pickup-requests
```

Commit previsto: `feat(api): add pickup requests`

## Objetivo

Implementar este bloque como entrega vertical de base de datos, API, autorizacion, auditoria/outbox, interfaz y pruebas, sin ampliar el alcance.

## Modelos

- PickupRequest
- PickupRequestItem

## Enums nuevos

- PickupRequestStatus: DRAFT, READY, COMPLETED, CANCELLED

## Cambios en enums existentes

_Ninguno._

## Permisos

- pickups.read
- pickups.manage

## Endpoints

- GET /pickup-requests
- POST /pickup-requests
- GET /pickup-requests/:pickupRequestId
- PATCH /pickup-requests/:pickupRequestId
- POST /pickup-requests/:pickupRequestId/ready
- POST /pickup-requests/:pickupRequestId/complete
- POST /pickup-requests/:pickupRequestId/cancel

## Rutas web

- /pickup-requests
- /pickup-requests/new
- /pickup-requests/[pickupRequestId]

## Reglas obligatorias

- Paquetes del mismo cliente y tenant.
- Facility permite pickup.
- Un paquete no aparece en dos solicitudes activas.
- READY congela items.
- COMPLETED registra actor y fecha.
- Validar elegibilidad financiera.

## Auditoria

- pickup_request.created
- pickup_request.ready
- pickup_request.completed
- pickup_request.cancelled

## Outbox

- pickup_request.ready
- pickup_request.completed

## Pruebas minimas

- Happy path y validaciones.
- Aislamiento multi-tenant.
- 401, 403, 404 y 409 cuando correspondan.
- Atomicidad y rollback con audit/outbox.
- Idempotencia de acciones repetibles.
- Componentes web y Playwright para el flujo principal.
- Limpieza especifica y sin handles abiertos.

## Fuera del alcance

- delivery a domicilio
- notificaciones automaticas

## Prompt 1 — Plan para Codex

```text
Lee primero AGENTS.md. Estamos trabajando en el Ticket 32: Solicitudes de retiro en sucursal.

No modifiques archivos. Inspecciona el repositorio y presenta un plan que cubra esquema, migraciones, modulos API/web, relaciones tenant-safe, DTOs, endpoints, permisos, auditoria, outbox, pruebas, archivos y validaciones.

Implementacion aprobada del ticket:
- Modelos: PickupRequest, PickupRequestItem
- Permisos: pickups.read, pickups.manage
- Endpoints: GET /pickup-requests, POST /pickup-requests, GET /pickup-requests/:pickupRequestId, PATCH /pickup-requests/:pickupRequestId, POST /pickup-requests/:pickupRequestId/ready, POST /pickup-requests/:pickupRequestId/complete, POST /pickup-requests/:pickupRequestId/cancel
- Rutas: /pickup-requests, /pickup-requests/new, /pickup-requests/[pickupRequestId]

Reglas obligatorias:
- Paquetes del mismo cliente y tenant.
- Facility permite pickup.
- Un paquete no aparece en dos solicitudes activas.
- READY congela items.
- COMPLETED registra actor y fecha.
- Validar elegibilidad financiera.

Fuera del alcance:
- delivery a domicilio
- notificaciones automaticas

Responde unicamente con el plan.
```

## Prompt 2 — Implementacion para Codex

```text
Apruebo el plan del Ticket 32. Implementa unicamente Solicitudes de retiro en sucursal.

1. Implementa exactamente los modelos, enums, permisos, endpoints y rutas aprobados.
2. Usa UUID, timestamptz(3), snake_case y relaciones tenant-safe.
3. Tenant y actor provienen del contexto autenticado, nunca del cliente.
4. Integra mutacion, audit y outbox en una misma transaccion cuando corresponda.
5. No registres secretos, tokens, cookies, credenciales ni PII innecesaria.
6. Recursos ajenos responden 404.
7. Agrega pruebas unitarias, integracion, e2e y frontend.
8. No modifiques Docker Compose sin aprobacion.
9. No hagas commits.
10. No implementes: delivery a domicilio, notificaciones automaticas.

Ejecuta db:format, db:validate, db:generate, db:migrate:status, db:check, rbac:sync-permissions, outbox:status, lint, typecheck, test, test:e2e, build y git diff --check.

Informa cambios, pruebas y resultados.
```

## Prompt 3 — Revision independiente

```text
Revisa el diff del Ticket 32 como ingeniero senior de NestJS, Prisma, PostgreSQL, Next.js, seguridad multi-tenant, auditoria y outbox. No modifiques archivos.

Verifica alcance exacto, relaciones tenant-safe, permisos, DTOs, estados, idempotencia, atomicidad, sanitizacion, respuestas, frontend /backend, pruebas, migracion reproducible y que no se implemento: delivery a domicilio, notificaciones automaticas.

Clasifica hallazgos como Critico, Alto, Medio o Bajo con archivo y ubicacion.
```

## Cierre

```powershell
git status
git diff --check
git add -A
git diff --cached --stat
git commit -m "feat(api): add pickup requests"
git status
```
